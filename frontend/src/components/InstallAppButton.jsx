...
function InstallAppButton({ alwaysVisible = false }) {
  ...
  if (!isPwaSupported && !alwaysVisible) return null;

  return (
    <Button
      onClick={handleInstallClick}
      variant="secondary"
      className="group text-foreground/70 flex h-fit items-center gap-1.5 rounded-full border-r border-b py-[3px] shadow-none transition-all duration-300 ease-in-out hover:-translate-y-0.5 hover:scale-101 hover:shadow-lg active:scale-95 has-[>svg]:px-3 sm:gap-2 dark:border-0 dark:py-[5px]"
    >
