...
function CardLG({ fund }) {
  return (
    <Link to={`/mutual-funds/${fund.scheme_code}`}>
      <Card className="h-44 w-48 cursor-pointer rounded-3xl justify-between gap-2 p-4 duration-200 hover:scale-101">
        <FundLogo fundHouseDomain={fund.detail_info} />
        <CardTitle className="line-clamp-2 text-sm leading-tight font-medium sm:font-[450] sm:text-[0.9rem]">
          {fund.short_name}
        </CardTitle>
