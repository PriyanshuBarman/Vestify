...
if (isCronEnvironment) {
  // Cron: Use partial schema. Typecast it to the strict schema so the rest
  // of the app doesn't have to deal with undefined types or use '!'.
  const result = envSchema.partial().safeParse(process.env);
  envConfig = (result.success ? result.data : {}) as z.infer<typeof envSchema>;
} else {
  // Prod/Dev: Strict parsing
