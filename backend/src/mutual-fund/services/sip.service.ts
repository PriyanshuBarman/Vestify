...
export const editSip = async ({
  ...
  sipDate,
  sipId,
}: EditSipSchema & { userId: string; sipId: string }) => {
...
export const skipSip = async (userId: string, sipId: string) => {
...
export const getSipDetail = async (sipId: string) => {
...
export const removeStepUp = async (sipId: string) => {
  return await db.mfSip.update({
    where: { id: sipId },
    data: {
      stepUpAmount: null,
